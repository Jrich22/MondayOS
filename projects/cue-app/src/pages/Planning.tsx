...
export default function Planning() {
  ...
  const plan = usePlan(event ?? ({ id: id ?? "" } as never));

  const now = Date.now();
  const report = useMemo(
    () => (event ? readiness(event, guests, plan, now) : null),
    [event, guests, plan, now],
  );
  const demand = useMemo(
    () => (event ? capacityDemand(event, guests) : null),
    [event, guests],
  );
  ...
  if (!event) {
    return (
      <div className="mx-auto max-w-3xl p-8">
        ...
        <Link to="/events" className="text-brand-400 hover:underline">← Back to events</Link>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6 sm:p-8">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2 text-sm text-ink-muted">
            <Link to={`/events/${event.id}`} className="hover:text-ink">← {event.title}</Link>
            <span>·</span>
        ...
        {report && (
          <span className={cn("rounded-xl border px-3 py-1.5 text-sm font-medium", STATE_STYLE[report.overall])}>
      ...
      <section className="card p-6">
        <h2 className="text-base font-semibold text-ink">Operational readiness</h2>
          ...
          {report?.categories.map((c) => (
            <div key={c.key} className="rounded-xl border border-line p-4">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className={cn("rounded-full border px-2 py-0.5 text-xs font-medium", STATE_STYLE[c.state])}>
                  {STATE_LABEL[c.state]}
                </span>
              </div>
              <p className="mt-2 text-sm text-ink-muted"><span className="text-ink-muted/70">Evidence:</span> {c.evidence}</p>
              <p className="mt-1 text-sm text-ink-muted"><span className="text-ink-muted/70">Next:</span> {c.nextAction}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {demand && (
        <section className="card p-6">
          <h2 className="text-base font-semibold text-ink">Capacity &amp; demand</h2>
          <p className="mt-0.5 text-sm text-ink-muted">
          ...
          {demand.warning && (
            <p className={cn("mt-4 rounded-xl border px-3 py-2 text-sm", STATE_STYLE[demand.state])}>
              ⚠ {demand.warning}
            </p>
          )}
        </section>
      ...
      {/* Ownership */}
      <section className="card p-6">
        <h2 className="text-base font-semibold text-ink">Planning ownership</h2>
        <p className="mt-0.5 text-sm text-ink-muted">
          A WorkspaceMember owns planning — distinct from People (relationship records). No production auth yet.
        </p>
        <div className="mt-4 flex items-center gap-3">
          <span className="grid h-9 w-9 place-items-center rounded-full border border-line text-xs font-semibold text-ink">
            {me.initials}
          </span>
      ...
      <Editor
        ...
        hint="Structure a multi-day event; days seed from the canonical event window."
        items={plan.days.map((d) => ({ id: d.id, primary: d.label, secondary: d.date }))}
        onAdd={(v) =>
          v.a &&
          update({ days: [...plan.days, { id: planId("day"), date: v.b || event.startsAt.slice(0, 10), label: v.a }] })
        }
        onRemove={(id) => update({ days: plan.days.filter((d) => d.id !== id) })}
        fields={[{ key: "a", placeholder: "Day label (e.g. Arrivals)" }, { key: "b", placeholder: "yyyy-mm-dd", type: "date" }]}
      />
      ...
      <section className="card p-6">
        ...
        <ul className="mt-3 space-y-2">
          {plan.milestones.map((m) => (
            <li key={m.id} className="flex items-center gap-3 rounded-xl border border-line px-3 py-2 text-sm">
              <button
              ...
              <span className={cn("flex-1 text-ink", m.status === "done" && "line-through text-ink-muted")}>{m.title}</span>
              {m.dueDate && <span className="text-ink-muted">{m.dueDate}</span>}
              <RemoveButton onClick={() => update({ milestones: plan.milestones.filter((x) => x.id !== m.id) })} />
            </li>
          ))}
          {plan.milestones.length === 0 && <li className="text-sm text-ink-muted">No milestones yet.</li>}
        </ul>
        <AddRow
          fields={[{ key: "a", placeholder: "Milestone (e.g. Confirm venue)" }, { key: "b", placeholder: "Due", type: "date" }]}
      ...
      <Editor
        ...
        hint="Accountability areas owned by a WorkspaceMember."
        items={plan.responsibilities.map((r) => ({ id: r.id, primary: r.area, secondary: r.ownerId ? me.name : "Unassigned" }))}
        onAdd={(v) => v.a && update({ responsibilities: [...plan.responsibilities, { id: planId("r"), area: v.a, ownerId: me.id }] })}
        onRemove={(id) => update({ responsibilities: plan.responsibilities.filter((r) => r.id !== id) })}
        fields={[{ key: "a", placeholder: "Area (e.g. Catering, AV, Comms)" }]}
      />

      {/* Risks & blockers */}
      <section className="card p-6">
        <h2 className="text-base font-semibold text-ink">Risks &amp; blockers</h2>
        <ul className="mt-3 space-y-2">
          {plan.risks.map((k) => (
            <li key={k.id} className="flex flex-wrap items-center gap-3 rounded-xl border border-line px-3 py-2 text-sm">
              <span className="flex-1 text-ink">{k.title}</span>
              <Badge className={k.severity === "high" ? "text-rose-300" : k.severity === "medium" ? "text-amber-300" : undefined}>{k.severity}</Badge>
              <button
                className={cn("rounded-lg border px-2 py-0.5 text-xs", k.blocker ? "border-rose-500/40 text-rose-300" : "border-line text-ink-muted")}
              ...
              <RemoveButton onClick={() => update({ risks: plan.risks.filter((x) => x.id !== k.id) })} />
            </li>
          ))}
...
function RemoveButton({ onClick }: { onClick: () => void }) {
  return (
    <button onClick={onClick} className="rounded-lg px-2 py-0.5 text-xs text-ink-muted hover:text-rose-300" aria-label="Remove">✕</button>
  );
}

type FieldSpec = { key: "a" | "b"; placeholder: string; type?: string };
...
function Editor({
  title, hint, items, fields, onAdd, onRemove,
}: {
  title: string;
  hint: string;
  ...
  return (
    <section className="card p-6">
      ...
      <ul className="mt-3 space-y-2">
        {items.map((it) => (
          <li key={it.id} className="flex items-center gap-3 rounded-xl border border-line px-3 py-2 text-sm">
            <span className="flex-1 text-ink">{it.primary}</span>
